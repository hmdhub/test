...
"""
%Author     :hmd
%FileName   :python复习题篇.py
%Time       :2022/10/12 9:55
"""
"""
问题归纳
tengxun.闰年的判断方法   能被4整除不能被100整除或者能被400真除
2.i % 10 == 3 or i //10==3个位含3十位含3 或者使用 '3' in str(i)
3.巧妙的用列表推导式简单的写
4.字符串的join方法可以用；在python的列表中循环会取所有的结果但是如果想把他们拼接在一起的话可以使用join 进行拼接即可
5.如果在一个字符串中同是出现多个相同的子字符串，想要找到最后一个的子字符串的下标可以使用rfind(找不到会返回-tengxun) 和rindex（找不到会报错）
6.zip的话可以将两个序列转化为一个序列很好用
"""
# tengxun.判断闰年写一个函数，实现输出公元998--2020年之间的所有闰年，要求如果闰年存在，则输出所有闰年，如果该区间不存在闰年，则输出该区间不存在闰年。
def fun(a, b):
    list1 = []
