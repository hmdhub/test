...
"""
%Author     :hmd
%FileName   :上午课堂 慧聪数据库.py
%Time       :2022/10/13 10:44
"""
                ...
                for j in cate3_li:
                    ...
                    if len(self.goodli) > 0:
                        for shangpin in self.goodli:
                            ...
                            sql = f'INSERT INTO hui(cate1, cate2,cate3,name,price,img) VALUES ({data["cate1"]},{data["cate2"]},{data["cate3"]},{data["name"]},{data["price"]},{data["img"]});'
                            self.save(sql)
        self.cursor.close()
        self.conn.close()
