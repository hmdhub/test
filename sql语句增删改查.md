...
--01 mysql 数据库的操作
    ...
    create database `db_test` charset=uf8;
    (如果数据库名字中存在-这种情况要是tab上边的符号将数据库的名字引起来作为一个整体)
...
--02 数据表的操作
    ...
    -- 创建表
    (数值类型)
    -- int tinyint  bit decimal float 
    -- float(5,3)代表一共有5位小数占3位
    -- int unsigned 无符号整形
    (字符串类型)
    -- varchar:表示不固定长度，比如varchar(3)填充内容时 数据不够3个字符有几个算几个
    varchar 在存取方面与char相反，存取就慢
    -- char: 表示固定长度 比如char(3)填充时 如果不够3个字符就会用空格补全
    char 长度固定，所以存取速度要比varchar快甚至快50%
    (时间日期类型)
    --date：日期 年月日
    --time: 时间 时分秒
    --datetime:年月日时分秒
    ...
    -- unique 表示唯一 给某个字段加了唯一约束，要求在这张表中该字段不能有重复的值
    -- default 默认值
   ...
   alter table classes add chongwu varchar(20) default"蛇";
    ...
    -- 修改表-修改字段：重命名版
    -- alter table 表名 change 旧字段名 新字段名 旧数据类型及约束;
    alter table classes change chongwu mascot varchar(20) default"法拉利";
    -- alter table 表名 change 旧字段名 新字段名  新数据；
...
+--------+-------------------------------------------+------+-----+---------+------------
        ...
        insert into students(name) values("老王"),("老张");
...
-- 分组
    ...
    -- having(注意having和group by)(对分组的结果进行判断-写到having后边)
...
-- 分页
    --select * from 表 limit 数字 只显示前几条数据
    -- limit(第n-1)*每页个数,每页的数据  
    -- limit start(代表从哪开始), count(代表显示个数)
...
select * from  表 where 条件 order by 字段 group by 字段 having 条件 limit 
